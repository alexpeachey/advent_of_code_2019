defmodule Advent.Day5b do
  ...
  def process([{_a, b, c, "01"}, p1, p2, dest | _], %{mem: mem, ptr: ptr}) do
    mem = compute(mem, {c, p1}, {b, p2}, dest, &Kernel.+/2)
    {:ok, %{mem: mem, ptr: ptr + 4}}
  end

  def process([{_a, b, c, "02"}, p1, p2, dest | _], %{mem: mem, ptr: ptr}) do
    mem = compute(mem, {c, p1}, {b, p2}, dest, &Kernel.*/2)
    {:ok, %{mem: mem, ptr: ptr + 4}}
  end

  def process([{_a, _b, _c, "03"}, dest | _], %{mem: mem, ptr: ptr}) do
    mem = input(mem, dest)
    {:ok, %{mem: mem, ptr: ptr + 2}}
  end

  def process([{_a, _b, c, "04"}, source | _], %{mem: mem, ptr: ptr}) do
  ...
  def process([{_a, b, c, "07"}, p1, p2, dest | _], %{mem: mem, ptr: ptr}) do
    mem = set_boolean(mem, {c, p1}, {b, p2}, dest, &Kernel.</2)
    {:ok, %{mem: mem, ptr: ptr + 4}}
  end

  def process([{_a, b, c, "08"}, p1, p2, dest | _], %{mem: mem, ptr: ptr}) do
    mem = set_boolean(mem, {c, p1}, {b, p2}, dest, &Kernel.==/2)
    {:ok, %{mem: mem, ptr: ptr + 4}}
  end
